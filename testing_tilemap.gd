...
func populate_tile_terrain():
	...
	if bugfixing != true:
		set_cells_terrain_connect(cells_Wall,0,0,true) #makes the auto-tiling work for generated stuff.
		set_cells_terrain_connect(cells_Ground,0,1,true)
		set_cells_terrain_connect(cells_Water,0,2,true)
		for tile in AllHallTiles:
			if River_Tiles_list.has(tile):
				set_cell(Vector2i(tile.x,tile.y),4, Vector2i(10,10)) #THIS IS THE BRIDGE TILE
